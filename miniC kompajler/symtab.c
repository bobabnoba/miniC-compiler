...
int insert_symbol(char *name, unsigned kind, unsigned type, 
  ...
  symbol_table[index].atr1 = atr1;
	for (int i = 0; i < 10; i++)
	//for(int i = 0; i < NELEMS(atr2); i++)
  	symbol_table[index].atr2[i] = atr2;
  return index;
}
...
void set_atr2(int index, int atr2_idx, unsigned atr2){
  if(index > -1 && index < SYMBOL_TABLE_LENGTH)
    symbol_table[index].atr2[atr2_idx] = atr2;
}

unsigned get_atr2(int index, int atr2_idx) {
  if(index > -1 && index < SYMBOL_TABLE_LENGTH)
    return symbol_table[index].atr2[atr2_idx];
  return NO_ATR;
}
...
void clear_symbols(unsigned begin_index) {
  ...
  for(i = begin_index; i < first_empty; i++) {
    ...
    symbol_table[i].atr1 = NO_ATR;
		//for(int k = 0; k < NELEMS(symbol_table[i].atr2); k++)
	for(int k = 0; k < 10; k++)
	    symbol_table[i].atr2[k] = NO_TYPE;
  }
